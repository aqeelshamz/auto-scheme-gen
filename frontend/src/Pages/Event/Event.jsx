...
const Event = (props) => {
  ...
  const assignMember = async () => {
    // var response = await api.post("/events/sectors/assign-member", {
    //   sectorId:
    // });
  };
  ...
  function isPointInsideSquare(point, square) {
    ...
    for (const sector of sectors) {
      if (sector.type === 1) {
        const south = sector.data.south;
        const west = sector.data.west;
        const north = sector.data.north;
        const east = sector.data.east;

        if (south <= lat && lat <= north && west <= lng && lng <= east) {
          return true;
        } else {
          return false;
        }
